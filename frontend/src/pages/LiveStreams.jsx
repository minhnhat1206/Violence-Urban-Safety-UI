...
// Bỏ enum CameraStatus → dùng string trực tiếp
      ...
      <div className="relative aspect-video">
        ...
        <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
          <button
            ...
            className="p-2 bg-slate-900/50 rounded-full text-white hover:bg-emerald-500"
          >
            <Maximize size={16} />
          </button>
        </div>
    ...
    const fetchStatus = () => {
      fetch('/api/camera-status')
        ...
        .then(({ cameras: statusMap }) => {
          ...
          setCameras(prev =>
            prev.map(cam => {
              const realStatus = statusMap[cam.id] ?? statusMap[cam.streamPath?.split('/').pop()];
              if (!realStatus || cam.status === 'OFFLINE') return cam;
